...
class OpenAdminQcsUploadHelper
{
    public static function buildQCSTempKeys($config)
    {
        ...
        $tempKeys = $sts->getTempKeys([
            ...
            'durationSeconds' => (int)$config['duration_seconds'], // 密钥有效期
            'allowPrefix' => $config['allow_prefix'], // 这里改成允许的路径前缀，可以根据自己网站的用户登录态判断允许上传的具体路径，例子： a.jpg 或者 a/* 或者 * (使用通配符*存在重大安全风险, 请谨慎评估使用)
